...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await db.collection("projectrequests").updateOne({ _id: id } as any, { $set: update });
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const [messages, total] = await Promise.all([
        db.collection("contacts").find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit).toArray(),
...
router.patch(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      await db.collection("contacts").updateOne(query, { $set: { status } });
